...
def bce_dynamic(pred: torch.Tensor, target: torch.Tensor, pos_ratio: 1., neg_ratio: 1., reduction = 'mean'):
    """动态BCE，对batch内的正负样本进行加权
    Args:
        pred (torch.Tensor): _description_
        target (torch.Tensor): _description_
        pos_ratio (1.): _description_
        neg_ratio (1.): _description_
        reduction (str, optional): _description_. Defaults to 'mean'.
    Returns:
    """
    pos_mask = torch.eq(target, 1).float()
    neg_mask = torch.eq(target, 0).float()
    weights = pos_mask * pos_ratio + neg_mask * neg_ratio
    return F.binaural_cross_entropy(pred, target, weights=weights, reduction=reduction)

class AsymmetricLoss(nn.Module):
    """Ref: https://link.zhihu.com/?target=https%3A//github.com/Alibaba-MIIL/ASL
